...
@section('content')
    <section class="py-12 bg-gray-50 min-h-screen">
        <div class="container mx-auto px-6">
            <h2 class="text-3xl font-bold text-gray-800 mb-8">Resident Profile</h2>
                ...
                <div class="bg-white rounded-xl shadow-md p-6 mb-6">
                    ...
                    <form action="{{ route('resident.profile.update') }}" method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="number" step="0.000001" name="latitude" id="latitude" value="{{ old('latitude', $residency->latitude ?? -26.2041) }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500">
                            @error('latitude')
                                <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            ...
                            <input type="number" step="0.000001" name="longitude" id="longitude" value="{{ old('longitude', $residency->longitude ?? 28.0473) }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500">
                            @error('longitude')
                                <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                            @enderror
                        </div>
            ...
            <div class="bg-white rounded-xl shadow-md p-6 mb-6">
                <h3 class="text-xl font-bold text-gray-800 mb-4">Council Membership Requests</h3>
                <form action="{{ route('resident.request_council') }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <label for="council_id" class="block text-gray-7
System: 00 font-medium mb-2">Request Council Membership</label>
                    ...
                    <button type="submit" class="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg font-bold transition duration-200">Request Membership</button>
                </form>
                <h4 class="text-lg font-semibold text-gray-800 mt-6 mb-4">Your Requests</h4>
                @if ($userRequests->isEmpty())
                    <p class="text-gray-600">No council membership requests found.</p>
                @else
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($userRequests as $request)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $request->council->name ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ ucfirst($request->status) }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $request->requested_at->format('Y-m-d H:i:s') }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
