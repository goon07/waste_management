...
<section class="py-12 bg-gray-100 min-h-screen">
    <div class="container mx-auto px-4 max-w-3xl">
        <h1 class="text-3xl font-bold mb-6">Edit Area: {{ $area->name }}</h1>
        ...
        <form action="{{ route('council.areas.update', $area) }}" method="POST" class="bg-white p-6 rounded shadow">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="3" class="mt-1 w-full border border-gray-300 rounded px-3 py-2">{{ old('description', $area->description) }}</textarea>
                @error('description') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
            </div>

            <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700">Update Area</button>
